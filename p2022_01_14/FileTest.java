...
public class FileTest {
	...
	public static void main(String[] args) {
		try {
			...
			// 반환함)
			System.out.println("create directory state : " + temp.mkdirs()); // 한번 생성후 false출력
			System.out.println("create directory state : " + tempFile.mkdirs()); // 생성되면 true

			// 1. 디렉토리 삭제 : 비어있는 디렉토리 삭제됨
//			tempFile.delete(); // test 디렉토리 삭제

			// 2. 비어있지 않는 디렉토리 삭제 : 파일이 비어있지 않으면 삭제 불가하다
			// 1) test 디렉토리 안에 있는 모든 파일을 구해와서 삭제
			...
			temp.delete(); // temp 디렉토리 삭제 : 자식 디렉토리인 temp만 삭제
			...
			System.out.println("temp isDirectory : " + temp.isDirectory()); // 경로면 true
			System.out.println("temp isFile : " + temp.isFile()); // 파일이면 true
		} catch (Exception e) {
			System.out.println("예외가 발생 헸습니다.");
			e.printStackTrace();
		}
	}
}
