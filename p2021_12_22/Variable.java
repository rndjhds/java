...
//	변수를 만드는 형식 : 자료형 변수명 = 데이터(값);
	...
	// 기본 자료형 변수 : 지역변수(Stack메모리 영역에 저장된다.)
		...
		// 1. 정수형 변수
			byte b1 = 10; // -128 ~ 127
//			byte b2 = 130; // 범위를 벗어날 경우 오뱌플로우 발생
			short s = 100; // -32768 ~ 32767
			int i = 1000; // -21억 ~ 21억
			long l = 10000; // int로 형변환되서 적용
			long ll = 2200000000L; // int형의 범의를 벗어날 경우 L를 붙여야 한다.
		...
		// 2. 실수형 변수
//			float f1 = 3.14;  // 오류 발생
			float f = (float)3.14; // 형변환 
			float f1 = 3.14f; // float형은 f를 붙여야 한다. 
			float f2 = 3.14F; // float형은 F를 붙여야 한다.
			double d = 42.195;
...
//			char c = 'AB'; // char형은 1글자만 저장 할 수 있다.
	...
	// 참조형 변수 : 클래스, 배열, 인터페이스
		...
		// 참조형 변수 : 배열 - 동알한 자료형의 데이터를 저장하는 정적인 자료구조
			int[] score = {80,90,100};
...
//			List list = List(); // 오류발생
			...
			for(int k = 0; k<list.size(); k++) {
				System.out.println(list.get(k)+"\t");  // \t는 tap키 누른것 처럼 간격 넓힘
			}
	}
